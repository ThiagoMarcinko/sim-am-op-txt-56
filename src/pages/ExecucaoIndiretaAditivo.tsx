...
const ExecucaoIndiretaAditivo = () => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.nrAditivoContrato || formData.nrAditivoContrato.length > 9) {
      toast({
        title: "Erro de Validação",
        description: "Número do Aditivo do Contrato deve ter no máximo 9 números",
        variant: "destructive",
      });
      return false;
    }

    if (!formData.nrAnoAditivo) {
      toast({
        title: "Erro de Validação",
        description: "Selecione o Ano do Aditivo",
        variant: "destructive",
      });
      return false;
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const content = `${formData.idPessoa}|${formData.cdIntervencao}|${formData.nrAnoIntervencao}|${formData.idOrigemAcompanhamento}|${formData.nrAcompanhamento}|${formData.idTipoAtoContrato}|${formData.idTipoOrigemContrato}|${formData.nrContrato}|${formData.nrAnoContrato}|${formData.nrCNPJOrigem}|${formData.nrAditivoContrato}|${formData.nrAnoAditivo}|`;

    const blob = new Blob([content], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'ExecucaoIndiretaAditivo.txt';
    ...
    toast({
      title: "Sucesso",
      description: "Arquivo ExecucaoIndiretaAditivo.txt gerado com sucesso!",
    });
  };
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium">Ano do Aditivo</label>
                    <Select value={formData.nrAnoAditivo} onValueChange={(value) => handleInputChange('nrAnoAditivo', value)}>
                      <SelectTrigger>
                        <SelectValue placeholder="Selecione o ano" />
                      </SelectTrigger>
                      <SelectContent>
...
export default ExecucaoIndiretaAditivo;
